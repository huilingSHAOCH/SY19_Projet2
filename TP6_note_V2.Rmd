...
cleanData<-data[,colSums(abs(data[,1:ncol(data)-1])) !=0]
trainIdx<-sample(1:N, nTrain)
train<-cleanData[trainIdx,]
test<-cleanData[-trainIdx,]

#FDA A CALCULER QUE POUR DONNEES TRAIN 
library("MASS")
lda_data<- lda(y~.,data=cleanData)
U<-lda_data$scaling
X<-as.matrix(cleanData[,1:ncol(cleanData)-1])
Z<-X%*%U
Z<-as.data.frame(Z)
y<-data$y
Z<-cbind(Z,y)
...
Selon le graph, on obtient l'erreur minimale 0.03703704 quand k = 7. En fait la méthode k plus proches voisins est une méthode non paramétrique et la donnée expressions a un grand nombre de prédicteurs mais peu de observations. Donc pour éviter le phénomène overfitting, il faut prendre une valeur relativement grande pour k (ici 7).
...
error
```
On obtient l'erreur 0.07407407. L'erreur estimée de QDA et LDA sont les mêmes. C'est à dire que les matrices de covariance de chaque class sont similaires.
