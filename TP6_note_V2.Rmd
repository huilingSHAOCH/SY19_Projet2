...
Le taux d'erreur est 0.18. Il est relativement proche de celui obtenu avec la LDA. Son erreur est comprise entre l'erreur de LDA(0.14) et QDA(0.4). Ceci est normal puisque Analyse discriminante régularisée est une régularisation entre LDA et QDA.
...
Pour les données suivantes, nous ne préciserons plus les codes pour calculer les test erreurs en raison de la contrainte sur la taille de rapport.
...
plot(nbvoisin,error,xlab="Nombre de voisin pris pour apprentissage",ylab="Erreur estimée")
```
Nous obtenons, par cross validation, une erreur minimale de 0.0784. Elle correspond cependant à 1 seul voisin. Pour les data_characteurs on a 17 prédicteurs et 10000 obsercations (p<<N). Donc c'est normal que la méthode avec plus de flexibilité (k = 1) est plus performante.
...
L'execution de la fonction tune est trop longue. On n'a pas pu run la fonction tune suffisamment longtemps pour obtenir un résultat.
...
L'erreurs de QDA, LDA et Bayésien naif sont très proches. Donc on peut déduire que les matrices de covariance de chaque class sont similaires et diagonales.
